...
def predict(transitions, emissions, words, tokens, prev, word):
    """
    ...
    for tag in tokens:
        if tag not in transitions[prev]:
            continue

        if word not in emissions[tag]:
            continue

        score = transitions[prev][tag] + emissions[tag][word]
...
def initParams(file):
    ...
    for u in tokens.union({'_START'}):
        for v in tokens.union({'_START'}):
            prev = (u, v)
            for w in tokens.union({'_STOP'}):
                if prev not in transitions:
                    transitions[prev] = {w: 0}
                else:
                    transitions[prev][w] = 0
...
for ds in datasets:
    ...
    outputFile = datafolder / "dev.p10.out"
